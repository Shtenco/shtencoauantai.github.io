...
async function traceCall(contract, method, args, overrides = {}) {
  ...
  const gasLimit = BigInt(overrides.gasLimit || GAS_LIMIT);
  try {
    await provider.call({ from, to: await contract.getAddress(), data: populated.data, gasLimit });
  } catch (error) {
...
describe("QuickSwap exact-transfer V3 diagnostic", function () {
  this.timeout(600000);

  it("executes ten cycles without constant-product violation", async function () {
    const { controller, token } = await deployFixture();
    for (let i = 0; i < 10; i += 1) {
      const block = await provider.getBlock("latest");
      console.log("V3_CYCLE", i, "robot", (await controller.robotUsdt()).toString(), "price", (await controller.spotPriceX18()).toString(), "scale", (await token.gonsPerFragment()).toString());
  ...
  it("executes bounded robot refill", async function () {
