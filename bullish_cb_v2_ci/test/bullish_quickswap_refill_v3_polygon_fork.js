...
describe("BullishQuickSwapCentralBankV2 + exact-transfer SYNA V3", function () {
  this.timeout(600000);
  ...
  it("makes the first ten cycles positive after full setup and cycle gas", async function () {
    ...
    const report = {
      scenario: "BULLISH_QUICKSWAP_EXACT_V3_FIRST10_POLYGON_FORK",
      chainId: 137,
    ...
    fs.writeFileSync(
      path.join(process.cwd(), "reports", "bullish_qs_v2_first10_fork.json"),
  ...
  it("executes bounded robot-capital refill and exact POL gas refill", async function () {
    ...
    const report = {
      scenario: "BULLISH_QUICKSWAP_EXACT_V3_DIRECT_REFILL_FORK",
      cycles: 1,
      robotRefillEvents,
      gasRefillEvents,
    ...
    fs.mkdirSync(path.join(process.cwd(), "reports"), { recursive: true });
    fs.writeFileSync(
      path.join(process.cwd(), "reports", "bullish_qs_v2_refill_fork.json"),
      JSON.stringify(report, null, 2)
    );
